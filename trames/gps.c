...
void init_uart0(void){
    ...
    UCTL1 &= ~SWRST;                     // Fin de l'état de reset de l'UART0

    // activation transmission et récéption
    ME1 |= URXE0 | UTXE0;                // Enable USART0 TXD/RXD
    IE1 |= URXIE0 ;                      // Enable USART0 RX interrup

    ME2 |= URXE1 | UTXE1;                // Enable USART0 TXD/RXD
    IE2 |= URXIE1 ;                      // Enable USART0 RX interrup

    // Config des ports
    P3SEL |= 0x30;                       // P3.4,5 = USART0 TXD/RXD
    P3DIR |= 0x20;                       // P3.4 output direction
    P3DIR &= ~0x10;                      // P3.5 output direction
    //P3DIR &= ~0x50;                      // P3.5 input direction
    P3SEL |= 0xC0;                       // P3.4,5 = USART0 TXD/RXD
    P3DIR |= 0x80;                       // P3.4 output direction
    P3DIR &= ~0x40;                      // P3.5 output direction
...
void usart0_rx (void) __interrupt[USART0RX_VECTOR]{  
    ...
    if(GPS_USB_ENABLE == 0){
       ...
       //debug_printf("0R: %s\n", buf_0);                   // RXBUF0 to TXBUF0 //Trame GPS
